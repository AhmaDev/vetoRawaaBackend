  ...
  connection.query(
    ...
    (err, result) => {
      if (result.length > 0) {
        ...
        connection.query(
          `SELECT *, (SELECT  IFNULL(CONCAT(itemType , ' ' , itemName,' ' , itemWeight, ' ' ,itemWeightSuffix, ' ' , ' * ' , ' ' , brand.brandName), item.itemName)  FROM item LEFT JOIN itemGroup ON item.itemGroupId = itemGroup.idItemGroup LEFT JOIN brand ON item.brandId = brand.idBrand LEFT JOIN itemType ON itemType.idItemType = item.itemTypeId WHERE idItem = invoiceContent.itemId) As itemName, (SELECT imagePath FROM item WHERE idItem = invoiceContent.itemId) As imagePath FROM invoiceContent WHERE invoiceId = ${req.params.id}`,
